...
The NVIDIA GeForce GTX TITAN X[[3]](#3) and NVIDIA GeForce RTX 3060[[2]](#2) GPUs are on paper pretty similar in terms of 64 bit FLOPs <br> (209 vs 199) and since the problem is compute bound, we expected about the same performance, which one can verify with the plots. (`T_eff ≈ 125 GB/s` for both GPUs, with a grid size of 256)
