...
But if we want to get the steady state solution:
...
```
As you can see in the plots below, using the above mentioned hardware, we are definitely compute bound.
...
Now that we have the optimal local problem size for a GPU, we will run a week scaling experiment using multiple GPUs. (4x NVIDIA GeForce GTX TITAN X)
